...
app.post('/supplier/new',(req,res)=>{
  ...
  var a = new Supplier({
    ...
    start_date:randomDate(1262378795,1420145195),
    end_date:randomDate(1514839595,1577911595),
    position: {
      lat:Math.random()*90,
      lng:Math.random()*180
    }
...
app.post('/supplier/bulk',(req,res)=>{
  var suppliers = req.body.supplier.map(s=>{
    return new Supplier({
      ...
      sup_rating:Math.floor(Math.random() * 100),
      bio_rating:Math.floor(Math.random() * 100),
      start_date:randomDate(1262378795000,1420145195000),
      end_date:randomDate(1514839595000,1577911595000),
      position: {
        lat:Math.random()*90,
        lng:Math.random()*180
      }
